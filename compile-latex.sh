...
[ $? != 0 ] && echo "Can't copy project files!" && exit -1
cd "$(find "$temp_dir" -type f -name "$doc_name.tex" -exec dirname {} \; -quit)"
src_dir="$(pwd)"
find . -type f -name '*.m' -exec octave --no-gui --norc -q -H {} \+
[ $? != 0 ] && echo "Can't compile octave figures!"
pdflatex -halt-on-error -output-directory="$src_dir" "$doc_name.tex"
[ $? != 0 ] && echo "Can't compile document (pass 1)!" && exit -1
biber --input-directory "$src_dir" --output-directory "$src_dir" "$doc_name"
[ $? != 0 ] && echo "Can't compile bibliography!"
pdflatex -halt-on-error -output-directory="$src_dir" "$doc_name.tex"
[ $? != 0 ] && echo "Can't compile document (pass 2)!" && exit -1
pdflatex -halt-on-error -output-directory="$src_dir" "$doc_name.tex"
[ $? != 0 ] && echo "Can't compile document (pass 3)!" && exit -1
find . -type f -name "$doc_name.pdf" -exec cp {} "$output_dir" \; -quit
echo "copied $doc_name.pdf to $output_dir/$doc_name.pdf"
exit 0
